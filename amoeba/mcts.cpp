...
bool MCTS::descend()
{
    ...
    for (;;)
    {
        ...
        if (m_edges[edgeIndex].childNode < 0)
        {
            ...
            amoeba::MoveResult result = amoeba::applyMove(
                ongoing->board, move, std::span{positionHistory.data(), positionHistorySize});
            ...
            m_pendingLeaf = std::get<amoeba::Board>(std::move(result));
            return true;
        }
...
const amoeba::Board* MCTS::pendingLeaf()
{
    ...
    while (std::get<OngoingNode>(m_nodes[0].contents).edgeCount != 0 &&
           static_cast<int>(m_nodes[0].visits) < m_config.simulations)
    {
        if (descend())
            return &*m_pendingLeaf;
    }
